...
const (
	VER_MAJOR_PROTO      = 0x1
	VER_COMMAND_SEARCH   = 0x113 // 0x11D for 2.1
	VER_COMMAND_EXCERPT  = 0x100
	VER_COMMAND_UPDATE   = 0x101 // 0x103 for 2.1
...
func (sc *Client) SetFilter(attr string, values []int, exclude bool) *Client {
	if attr == "" {
...
func (sc *Client) SetFilterRange(attr string, umin, umax int, exclude bool) *Client {
	if attr == "" {
...
func (sc *Client) AddQuery(query, index, comment string) (i int, err error) {
	...
	for _, f := range sc.filters {
		...
		case SPH_FILTER_RANGE:
			req = writeInt32ToBytes(req, f.umin)
			req = writeInt32ToBytes(req, f.umax)
...
func (sc *Client) RunQueries() (results []Result, err error) {
	...
	nreqs := len(sc.reqs)
	var allReqs, reqs []byte
	for _, req := range sc.reqs {
		allReqs = append(allReqs, req...)
	}
	reqs = writeInt32ToBytes(reqs, len(allReqs)+4) // it's a client
	reqs = writeInt32ToBytes(reqs, nreqs)
	reqs = append(reqs, allReqs...)
	response, err := sc.doRequest(SEARCHD_COMMAND_SEARCH, VER_COMMAND_SEARCH, reqs)
	if err != nil {
		return nil, err
	}
